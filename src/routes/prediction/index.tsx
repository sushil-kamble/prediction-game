...
import { ArrowRight, LinkIcon, Sparkles, Trophy, Zap } from "lucide-react";
...
export const Route = createFileRoute("/prediction/")({
	head: () => ({
		meta: [
			{
				title: "Prediction Game | Sushil Games",
			},
		],
	}),
	component: PredictionHomeRoute,
});

function PredictionHomeRoute() {
	...
	function parseChallengeId(input: string): string | null {
		...
		try {
			const url = new URL(
				trimmed.startsWith("http")
					? trimmed
					: `https://placeholder.com${trimmed.startsWith("/") ? trimmed : `/${trimmed}`}`
			);
			...
			// Not a URL — treat as raw challenge ID
		}
	...
	function handleJoinChallenge(e: React.FormEvent) {
		e.preventDefault();
		setJoinError(null);

		const challengeId = parseChallengeId(joinUrl);
		if (!challengeId) {
			setJoinError(
				"Couldn't find a valid challenge link. Paste the full URL you received."
			);
			return;
		}

		navigate({ to: "/prediction/c/$challengeId", params: { challengeId } });
	}
	...
	return (
		<PageShell className="gap-8 py-8 sm:py-16">
				...
				<div className="grid gap-12 lg:grid-cols-[1.2fr_0.8fr]">
					<div className="flex flex-col justify-center">
						...
						<p className="border-primary mt-2 max-w-xl border-l-4 pl-4 text-lg leading-relaxed font-medium text-zinc-300 sm:text-xl">
							Build a prediction challenge in seconds. Share one link with your
							crew. Watch them fight for the top spot — live, loud, and in real
							time.
						</p>
						<div className="mt-10 flex flex-col gap-4 sm:flex-row">
							...
							<Button variant="outline" size="lg" asChild>
								<a href="#join-challenge" className="no-underline">
									JOIN CHALLENGE
								</a>
							</Button>
						</div>
						...
						{[
							{
								title: "THUMB-READY",
								description:
									"Fat targets. Zero friction. Made for match-day chaos.",
								icon: <Zap className="h-6 w-6 text-black" />,
							},
							{
								title: "LIVE SCORES",
								description:
									"Rankings shift the second you score. No refresh needed.",
								icon: <Trophy className="h-6 w-6 text-black" />,
							},
							{
								title: "ONE LINK",
								description:
									"Share to any group chat. Rich previews. Instant access.",
			...
			<div
				id="join-challenge"
				className="relative mt-8 border-2 border-zinc-800 bg-black p-6 sm:p-10"
			>
				<div className="mb-8">
					<div className="text-primary mb-2 text-sm font-bold tracking-widest uppercase">
						Got a Link?
					...
					<p className="mt-4 max-w-2xl text-base leading-relaxed text-zinc-400">
						Paste the challenge link your friend shared and jump straight into
						the action.
					</p>
				</div>
				...
				>
					<div className="flex-1">
						<div className="relative">
							<LinkIcon className="pointer-events-none absolute top-1/2 left-4 h-5 w-5 -translate-y-1/2 text-zinc-500" />
							<Input
								type="text"
								value={joinUrl}
								onChange={(e) => {
						...
						{joinError && (
							<p className="mt-2 text-sm font-bold tracking-wide text-red-400">
								{joinError}
							</p>
						)}
					</div>
					<Button type="submit" size="lg" className="sm:h-14">
						JOIN NOW
						<ArrowRight className="ml-2 h-5 w-5" />
					</Button>
				</form>
			...
			<div className="mt-8 grid gap-6 sm:grid-cols-[1fr_1.5fr]">
				<div className="border-primary bg-primary border-2 p-8 text-black shadow-[8px_8px_0px_0px_rgba(255,255,255,0.2)]">
					<p className="mb-4 text-sm font-extrabold tracking-[0.2em] uppercase">
						ANY SPORT // YOUR RULES
					</p>
					<h3 className="font-display mb-6 text-4xl leading-tight uppercase">
						BUILT FOR GAME DAY
					</h3>
					<p className="text-base leading-relaxed font-bold">
						Cricket to F1. Football to basketball. If there's a match, there's a
						challenge. Spin one up in under a minute.
					</p>
				</div>
